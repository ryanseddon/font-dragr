...
(function(win, doc){
    ...
    function buildFontListItem( event, name, size, data, type ) {
        ...
        if(type === "gwf") {
            // Special case for loading google webfonts (gwf) fonts directly
            load( type, name, type );
        } else {
    ...
    // Inspired by jsconsole.com cmd code.
    function runCMD(cmd) {
        var parts = [], fn;
        parts = cmd.split(' ');
        fn = parts.shift();
    ...
    function load(service, font, special) {
        if(service === "gwf") {
            // Google web fonts
            var link = document.createElement("link"), len, name;
            link.rel = "stylesheet";
    ...
    function parseDataFont( data ) {
        ...
        try {
            ...
            if( !data.error ) {
                ...
                var fontFileName = (data.fontType === "gwf") ? data.fontName : data.fontName.split( "/" )
                                                                                .reverse()[0]
                                                                                .replace( /\..+$/, "" )  // Remove file extension
                                                                                .replace( /\W+/g, "-" ); // Replace any non alpha numeric characters with -
                ...
                if( /*@cc_on!@*/0 && !!FD.ltIE9 || data.fontType === "gwf" ) {
